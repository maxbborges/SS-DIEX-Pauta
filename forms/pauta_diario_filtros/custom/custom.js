...
function init(){
    ...
    $('[name="txt_horario"]').text(dia+'/'+mes+'/'+ano+' - '+hora+':'+minuto)
    if (FORM_MODE=='VIEW'){
    ...
    setTimeout(()=>{
        for(i=0;i<dataset.length;i++){
            ...
            if(status[status.length-1].status==4){
                ...
                if (slc_status!='Cancelada'&&slc_status!='Todos'){
                    continue;
                }
            } else {
                if (slc_status=='Cancelada'){
                    continue;
                }
            }
